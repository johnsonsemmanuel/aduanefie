import { alpha, Grid, styled, Tooltip, Typography } from "@mui/material";
...
const StoreCard = (props) => {
	...
	const { ref: textRef2, isEllipsed: isEllipsed2 } = useTextEllipsis(
		item?.name,
	);
			...
			{topoffer ? (
				<ContentSection>
					...
					>
						...
						{Number(item?.avg_rating) > 0 ? <CustomRatingBox rating={item?.avg_rating} /> : null}
					</Box>
					<Box
						sx={{
							display: "flex",
				...
				<CustomBoxFullWidth>
					<Grid container>
						...
						<Grid item xs="auto" sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
							{item?.avg_rating > 0 && (
