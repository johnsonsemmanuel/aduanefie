...
export const Taxi = ({ isLoading, label, color, configData }) => {
  const [sideDrawerOpen, setSideDrawerOpen] = useState(false);
        ...
        icon={
          ...
          >
            <DirectionsCarOutlinedIcon
              sx={{ fontSize: "20px", color: color || "inherit" }}
            />
            {label && (
              <Typography
                sx={{
...
const SecondNavBar = ({ configData }) => {
  ...
  const { openForgotPasswordModal, openSignInModal } = useSelector(
  ...
  // Reopens the sign-in modal whenever something dispatches
        ...
        >
          {!token && location && (
            <IconButton onClick={handleTrackOrder} id="track-order-button">
              <Tooltip
                title={
                  moduleType !== "rental" ? t("Track order") : t("Track Trip")
                }
                arrow
                placement="top"
                componentsProps={{
          ...
          {moduleType === "rental" && (
            <Taxi isLoading={isLoading} configData={configData} />
          )}
