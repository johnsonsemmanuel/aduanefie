    ...
    <div className="space-y-4 min-w-0">
      ...
      <div className="flex gap-1 overflow-x-auto scrollbar-thin pb-1">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-medium whitespace-nowrap transition-colors ${activeTab === 'faq' ? 'bg-primary text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
        >
          <HelpCircle className="w-3.5 h-3.5" /> FAQ
        </button>
        <button
          onClick={() => setActiveTab('articles')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-medium whitespace-nowrap transition-colors ${activeTab === 'articles' ? 'bg-primary text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
        >
          <FileText className="w-3.5 h-3.5" /> Knowledge Base
        </button>
        <button
          onClick={() => setActiveTab('contact')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-medium whitespace-nowrap transition-colors ${activeTab === 'contact' ? 'bg-primary text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
        >
                ...
                return (
                  <div key={faq.id}>
                    <button
                      ...
                      className="w-full text-left px-4 py-3 flex items-start justify-between gap-3 hover:bg-surface-hover transition-colors"
                    >
            ...
            filteredArticles.map(article => {
              ...
              return (
                <button
                  ...
                  className="text-left rounded-lg border border-border bg-surface p-4 hover:bg-surface-hover transition-colors group"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <GlassCard>
            ...
            <button
              ...
              className="w-full px-4 py-2.5 rounded-full bg-primary text-white text-xs font-semibold inline-flex items-center justify-center gap-1.5 hover:bg-primary/90 transition-colors"
            >
      ...
      <Modal open={ticketOpen} onClose={() => setTicketOpen(false)} title="Submit Support Ticket" size="md">
        <form onSubmit={handleTicketSubmit} className="space-y-3">
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setTicketOpen(false)} className="px-4 py-2 rounded-full border border-border text-text-secondary text-sm hover:bg-surface-hover transition-colors">Cancel</button>
            <button type="submit" className="px-4 py-2 rounded-full bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors">Submit Ticket</button>
          </div>
        </form>
      </Modal>
    </div>
